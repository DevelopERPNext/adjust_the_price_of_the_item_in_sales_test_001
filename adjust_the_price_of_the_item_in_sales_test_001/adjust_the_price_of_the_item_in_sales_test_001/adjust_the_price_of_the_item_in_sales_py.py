        ...
        if item_doc.foreign_purchases_items_a_001:
            ...
            if avg_purchase_price is not None:
                ...
                if item_price_name:
                    ...
                    if hasattr(item_price_doc, 'selling') and item_price_doc.selling == 1:
                        item_price_doc.price_list_rate = new_price
                    ...
                    if "Standard Selling" in ["Standard Selling", "البيع القياسية"]:
                        new_item_price.price_list = "Standard Selling"
                    else:
                        new_item_price.price_list = "البيع القياسية"
...
def calculate_average_purchase_price(item_code):
    ...
    if purchase_invoices and purchase_invoices[0].avg_rate is not None:
        avg_price = flt(purchase_invoices[0].avg_rate)
        # frappe.msgprint(f"Average Purchase Price for item {item_code}: {avg_price}")
        return avg_price

    # frappe.msgprint(f"No purchase data found for item code: {item_code}.")
    # frappe.msgprint(str("========================"))
    # frappe.msgprint(frappe.as_json(purchase_invoices))
